...
tournamentActionHandler.handleRegistration = function(params) {
    ...
    if(!err) {
      broadcastHandler.fireBroadcastToAllSessions({app: params.self.app, data: {_id: params.request.tournamentId, updated: {enrolledPlayers: (result || 0)}, event: stateOfX.recordChange.tournamentEnrolledPlayers}, route: stateOfX.broadcasts.tournamentTableUpdate});      
    } else {
...
tournamentActionHandler.handleTournamentState = function(params) {
  broadcastHandler.fireBroadcastToAllSessions({app: params.self.app, data: {_id: params.tournamentId, updated:{state: params.tournamentState}, event: stateOfX.recordChange.tournamentStateChanged}, route: stateOfX.broadcasts.tournamentStateChange});  
  if(params.tournamentState === stateOfX.tournamentState.finished){
    ...
    setTimeout(function() {
      broadcastHandler.fireBroadcastToAllSessions({app: params.self.app, data: {_id: params.tournamentId, state: stateOfX.tournamentState.register, event: stateOfX.recordChange.tournamentStateChanged}, route: stateOfX.broadcasts.tournamentStateChange});
...
tournamentActionHandler.handleDynamicRanks = function(params) {
  ...
  imdb.getRanks({tournamentId: params.tournamentId, gameVersionCount: params.gameVersionCount}, function(err, result) {
    ...
    if(!err) {
      broadcastHandler.fireBroadcastToAllSessions({app: params.self.app, data: {tournamentId: params.tournamentId, updated: {ranks: result.ranks} , gameVesionCount:result.gameVersionCount, event: stateOfX.recordChange.tournamentRankUpdate}, route: stateOfX.broadcasts.tournamentRankUpdate});
    } else {
...
tournamentActionHandler.prizePool = function(params){
  ...
  prizePoolHandler.calculatePrizePool(newParams, function(err, result){
    ...
    broadcastHandler.fireBroadcastToAllSessions({app: params.self.app, data: {tournamentId : params.tournamentId, updated: {prizePool: result, channelId: params.channelId}, event: stateOfX.recordChange.prizePool}, route: stateOfX.broadcasts.tournamentLobby});
