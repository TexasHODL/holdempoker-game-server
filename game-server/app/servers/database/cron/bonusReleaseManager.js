    ...
    db.findUsersOpts(query, projection, function (err, result) {
        if (err) {
            ...
            if (result.length !== 0) {
                ...
                cb(null, { success: true, info: 'No one is getting Vip points released, so yes WORK DONE!' });
            }
        }
    });
};
    ...
    db.findUserBonusDetails(query, function (err, result) {
        if (err) {
            ...
            if (result.bonus.length !== 0) {
                ...
                const x = _.find(result.bonus, function (object) { return object.unClaimedBonus > 0 && object.expireStatus == 0; });
                if (x) {
                    //player have at lease one unclaimed bonus present
                    console.log("player have at lease one unclaimed bonus present"+ player.userName);
                    player.bonusUsedList = result.bonus;
                    cb(null, player, params);
                } else {
                    //player have no unclaimed locked bonus present.
                    console.log("This player has no Unclaimed locked Bonus to release." + player.userName);
                    cb(null, { success: true, info: "This player has no Unclaimed locked Bonus to release." }, params);
                }
            } else {
                //player haven't used any bonus code till now
                console.log("This player has not used any bonus till nw." + player.userName);
                cb(null, { success: true, info: "This player has not used any bonus till nw." }, params);
            }
        }
    });
};
...
const processBonusRelease = (player, params, cb) => {
    ...
    // }
    console.log("player");
    async.eachSeries(player.bonusUsedList, function (bonus, ecb) {
        // async.waterfall([
        ...
        if (bonus.unClaimedBonus > 0 && bonus.expireStatus == 0 && !player.exit) {
            var tempAmt = (configConstants.percentForBonusRelease * bonus.unClaimedBonus) / 100;
            if (player.statistics.countPointsForBonus >= tempAmt) {
                ...
                passbookData.prevAmt = player.realChips + player.instantBonusAmount + player.amountReleased - bonus.claimedBonus;
                passbookData.amount = bonus.claimedBonus;
                passbookData.newAmt = player.realChips + bonus.claimedBonus + player.instantBonusAmount + player.amountReleased - bonus.claimedBonus;
...
const updatePlayerAccountAndBonus = (player, params, cb) => {
    ...
    const query = { playerId: player.playerId };
    const updateData = { $inc: { realChips: player.amountReleased }, $set: { statistics: player.statistics } };
    db.returnRealChipsToPlayer(query, updateData, function (err, result) {
        if (err) {
            ...
            db.updateBounsDataSetKeys(query, { bonus: player.bonusUsedList }, function (err, result) {
                if (err) {
...
const updateExpiryDocuments = (player, params, cb) => {
    ...
    const query = { playerId: player.playerId, uniqueId: { $in: player.uniqueIdArray } };
...
const processEveryPlayer = (params, cb) => {
    ...
    if (params.success) {
        ...
        async.eachSeries(params.playerList, function (player, ecb) {
            async.waterfall([
