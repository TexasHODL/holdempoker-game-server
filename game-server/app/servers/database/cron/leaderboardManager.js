...
var sendMailAndMessageToWinners = (player, leaderboard) => {
	...
	// if (leaderboard.leaderboardType == 'openVip' || leaderboard.leaderboardType == 'closedVip') {
	const rank = ["1st", "2nd", "3rd", "4th", "5th", "6th", "7th", "8th", "9th", "10th", "11th", "12th","13th", "14th", "15th", "16th", "17th", "18th", "19th", "20th"];
	console.log("send mail to winners"+ JSON.stringify(player));
	let blindsArray = [];
	for (let i = 0; i < leaderboard.tables.length; i++) {
		var blinds = leaderboard.tables[i].smallBlind + "/" + leaderboard.tables[i].bigBlind;
	...
	pomelo.app.rpc.connector.adminManagerRemote.sendLeaderboardWinnerMail("", mailData, function (response) {
		console.log("response in send mail", response);
	});
	var msgData = {};
	msgData.mobileNumber = player.mobile;
	msgData.msg = 'Dear ' + player._id.userName + ', The result of the ' + msgText+' RACE has been announced. Kindly check your email to see the result or visit at www.pokermoogley.com';
	pomelo.app.rpc.connector.adminManagerRemote.sendMsgToLeaderboardWinners("", msgData, function (response) {
...
var addAmountInPlayerAccount = (player) => {
	...
	profileMgmt.addChips(data, function (response) {
		if (response.success) {
			...
			financeDb.updateBalanceSheet({ $inc: { leaderboardWinning: player.amountWon || 0 } }, function (err, result) {
				console.log("Balance sheet updated");
			});
			//update total leaderboard winnings of the player
			db.returnRealChipsToPlayer({ playerId: player._id.pId }, { $inc: { totalLeaderboardWinnings: player.amountWon } }, function (err, result1) {
				console.log("if errr " + err);
			});
		} else {
			process.emit('forceMail', { title: "for leaderboard-winnings", data: player });
		}
	});
};
...
var checkParticipantsAndAssignRank = (leaderboard, params, cb) => {
	console.log("Inside filter player min vip points" + JSON.stringify(leaderboard));
	...
	let i = 1;
	async.eachSeries(leaderboard.participantsArray, function (player, ecb) {
		console.log("player " + i);
		console.log(JSON.stringify(player));
		if (leaderboard.winnerDeclared) {
			if ((leaderboard.leaderboardType == "openVip" || leaderboard.leaderboardType == "closedVip") && player.total >= leaderboard.minVipPoints){
				player.amountWon = leaderboard.payout[i - 1] || 0;
				player.rank = i <= leaderboard.noOfWinners ? i : "N/A";
			} else if ((leaderboard.leaderboardType == "openHand" || leaderboard.leaderboardType == "closedHand") && player.total >= leaderboard.minHands){
				player.amountWon = leaderboard.payout[i - 1] || 0;
				player.rank = i <= leaderboard.noOfWinners ? i : "N/A";
			}else{
				player.amountWon = 0;
...
var updatePlayerBalanceAndInform = (leaderboard, params, cb) => {
	async.each(leaderboard.participantsArray, function (player, ecb) {
...
var setLeaderboardToExpired = (leaderboard, params, cb) => {
	...
	adminDb.updateLeaderboard(query, updateData, function (err, result) {
		if (err) {
			console.log("Error while updating leaderboard status to Expired" + leaderboard.leaderboardId);
			cb("Error while updating leaderboard status to Expired" + leaderboard.leaderboardId);
		} else {
			cb(null, leaderboard, params);
		}
	});
...
var forEveryLeaderboard = (params, cb) => {
	...
	if (params.leaderboardList.length > 0) {
		async.eachSeries(params.leaderboardList, function (leaderboard, ecb) {
			console.log("Leaderboard going to process" + leaderboard);
			if ((leaderboard.leaderboardType == 'openVip' || leaderboard.leaderboardType == 'openHand')) {
				async.waterfall([
					...
					if (err) {
						...
						process.emit('forceMail', { title: "for leaderboard", data: err });
					}
					ecb(null);
				});
			} else if ((leaderboard.leaderboardType == 'closedVip' || leaderboard.leaderboardType == 'closedHand')) {
				async.waterfall([
...
const assignBonusPlayerInParticipants = (leaderboard, params, cb) => {
	const remainingPlayers = _.difference(_.pluck(leaderboard.playerBonusList, 'playerId'), _.pluck(_.pluck(leaderboard.participantsArray, '_id'), 'pId'));
	if (remainingPlayers.length > 0) {
		async.each(remainingPlayers, function (playerId, ecb) {
...
var changeStatusofUpcomingLeaderboard = (params, cb) => {
	...
	adminDb.changeStatusLeaderboard(query, updateData, function (err, result) {
		if (err) {
			...
			process.emit('forceMail', { title: "for leaderboard", data: "The status for upcoming leaderboard is not changed." });
		} else {
			cb(null, params);
		}
	});
