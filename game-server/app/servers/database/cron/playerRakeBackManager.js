...
var UpdateEachPlayerRealChips = function (params, cb) {
	...
	async.each(params.rakeBackData, function (rakeData, ecb) {
		...
		profileMgmt.addChips(data,function(response){
			if(response.success){
				var content = {};
				content.playerName = rakeData.rakeByName + " (" + rakeData.rakeByUsername + ")";
				content.date = new Date(rakeData.addedDate + (330*60*1000)).toLocaleString() + "(IST)";
				...
				var query = {id: rakeData._id, prevBalance : response.previousBal, newBalance: response.newBalance};
				financeDB.addBalanceDetailsInRakeback(query, function(err, updateResult){
					console.trace("Balance updated in Player Rake back");
				});
			}else{
				process.emit('forceMail', {title:"for rakeback-add-chips-failed", data: rakeData});
				ecb(null);
			}
		});
	}, function (err) {
...
var sendRakebackMail = function(params, cb){
	pomelo.app.rpc.connector.adminManagerRemote.sendRakebackMail("", params, function (sendRakebackMailResponse) {
