...
function enableAll(app, message) {
	...
	stateOfServer.disableLogin.byScheduleId = message.meta && message.meta.scheduleId;
	...
	stateOfServer.disableGameStart.byScheduleId = message.meta && message.meta.scheduleId;
	stateOfServer.disableGameStart.timestamp = new Date().getTime();

	stateOfServer.disableJoin = {};
	stateOfServer.disableJoin.status = false;
	stateOfServer.disableJoin.byScheduleId = message.meta && message.meta.scheduleId;
	stateOfServer.disableJoin.timestamp = new Date().getTime();

	stateOfServer.disableSit = {};
	stateOfServer.disableSit.status = false;
	stateOfServer.disableSit.byScheduleId = message.meta && message.meta.scheduleId;
	stateOfServer.disableSit.timestamp = new Date().getTime();

	app.set('serverStates', stateOfServer);

	dropMail({subject: "Server Up Process: "+configConstants.mailFromAppName, msg: "Reporting from different server instances. This server has enabled all blocked tasks- login, join, sit, gamestart. Check if any server(s) have not reported in near time same message."});

	var query = {type: 'serverStates', serverId: app.serverId};
	adminDb.updateServerStates(query, {$set: stateOfServer, $setOnInsert: query}, function (err, result) {
...
function informServers(app, message, cb) {
	...
	if (receiverServerIds.length > 0) {
		...
		for (var i = 0; i < receiverServerIds.length; i++) {
			app.rpcInvoke(receiverServerIds[i], {namespace: namespace, service: service, method: method, args: [message]}, function (argument) {
				count--;
...
module.exports.fetchServerState = function (cb) {
	adminDb.findAllServerStates({type: 'serverStates'}, function (err, result) {
		if (err || !result) {
			...
			if (result.length<0) {
				cb({success: true, status: true, info: "Everything is up and running."});
			} else {
				var status = true;
				...
				if (status) {
					...
					cb({success: true, status: false, info: "Some/all servers are under maintenance, and has disabled some features. Click 'Switch to running' to make all servers up and running. By clicking on this, you understand its risks and all impacts."});
				}
			}
		}
	});
...
// altogether
var renderLeaveOnClient = function (actionHandler, params, item) { // error in channelId
	setTimeout(function (actionHandler, params, item) {
		actionHandler.handleLeave({session: {}, channel: params.channel,
	...
	if (app.serverType == 'room') {
		...
		imdb.getAllTable({serverId: app.serverId}, function (err, tables) {
			...
			for (var i = 0; i < params.tables.length; i++) {
				...
				var channel = app.get('channelService').getChannel(params.tables[i].channelId);
				if(channel){
					setTimeout(function (channel) {
						channel.pushMessage(/*'channelInfo'*/ 'playerInfo', {channelId: (channel.name), heading: 'Server down',
						info: 'Everybody will be made leave his/her seat, forcefully. Server is going under maintenance.', buttonCode: 1});
					}, 50, channel);
				}
			}
			...
			async.eachSeries(params.tables, function (table, ecb) {
				async.eachSeries(table.players, function (player, eccb) {
					app.rpc.database.tableRemote.leave({}, {isRequested: false, playerId: player.playerId, channelId: table.channelId, isStandup: false, playerName: player.playerName}, function (leaveResponse) {
						if (leaveResponse.success) {
							params.totalLeaveSuccess += 1;
							var channel = app.get('channelService').getChannel(table.channelId);
							if (channel) {
								renderLeaveOnClient(actionHandler, {channel: channel, channelId: table.channelId}, {playerId: player.playerId, playerName: player.playerName});
							}
						} else {
				...
				if (params.totalSittingPlayers == (params.totalLeaveSuccess+params.totalLeavefail) ) {
					logger('serverId: '+app.serverId+', all leave requests completed.');
					if (params.totalSittingPlayers == params.totalLeaveSuccess) {
						logger('serverId: '+app.serverId+', all leave requests were successful.');
					}
					if (params.totalLeavefail>0) {
						logger('serverId: '+app.serverId+', some leave were failed - ', params.totalLeavefail);
						logger('serverId: '+app.serverId+', trying again in few minutes - ', params.totalLeavefail);
						///////////////
						setTimeout(startForcedLeave.bind(null, app, scheduleId), (confServerDown.iterativeForceLeaveAfter_Minutes||2)*60*1000);
					}
				}
			});
...
var kickSessions = function (app, data) {
	dropMail({subject: "Scheduling Server Down - "+configConstants.mailFromAppName+": Kicking Users Started", msg: "Players will be made force LOG-OUT now. Reporting from different (connector) server instances. This is last step of SERVER DOWN PROCESS. Check for a clean cluster and PUT IT DOWN."});
...
module.exports.startForcedLeave = function (app, scheduleId) {
	...
	informServers(app, message, function (response) {
		logger('all messages sent, for login disable');
		setTimeout(function (app, message) {
			startKickingSessions(app, message);
		}, (confServerDown.startKickSessionAfterStartForceLeave_Minutes||2)*60*1000, app, message);
	});
};
...
module.exports.msgRcvd = function (app, message, cb) {
	...
	switch(message.title){
		case 'enableAll' : enableAll(app, message); break;
		case 'disableLogin' : disableLogin(app, message); break;
		case 'disableGameStart' : disableGameStart(app, message); break;
		case 'forceLeave' : startForcedLeave(app, message); disableJoin(app, message); disableSit(app, message); break;
		case 'kickSessions' : kickSessions(app, message); break;
		...
		default: return cb('message title is invalid');
	}
	cb(null);
...
module.exports.checkServerState = function (event, app) {
	...
	switch(event){
		...
		case 'gameStart' : 
			stateOfServer = app.get('serverStates') || {};
			if(stateOfServer.disableGameStart && stateOfServer.disableGameStart.status){
				return true;
			}
			return false;
		break;
...
module.exports.checkClientStatus = function (event, msg, app, cb) {
	...
	adminDb.findGameVersions({deviceType: msg.deviceType, appVersion: msg.appVersion}, function (err, result) {
		if (err || !result) {
			return cb({success: false});
		} else {
			if (result.length>0) {
				...
				if (!!result.isUpdateRequired) {
					...
					if (msg.deviceType == "website" || msg.deviceType == "browser") {
						...
						infoUpdateNeeded = "An updated version is required to continue playing the game. Kindly download the updated build.";
					} else {
						infoUpdateNeeded = "An updated version is required to continue playing the game. Kindly download/reload the game.";
					}
					return cb({success: false, info: infoUpdateNeeded, errorType: "5011" /*update game code*/});
				} else if (!!result.isInMaintainance) {
					return cb({success: false, info: "Server is under maintenance. Please try again later."});
				} else {
					return cb(null, {success: true});
				}
			} else {
