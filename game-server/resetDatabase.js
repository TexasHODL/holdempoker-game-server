...
resetDatabase.reset = function () {
	...
	inmemoryDb.insertLog({log: "This is a test log and will be deleted after " + parseInt(configConstants.expireLogSeconds) + " seconds."}, function(err, res){
		console.log('Test log has been inserted properly!');
		setTimeout(function(){
			mongodb.inMemoryDb.collection('infoLogs').createIndex( { "createdAt": 1 }, { expireAfterSeconds: parseInt(configConstants.expireLogSeconds) });
	...
	mongodb.inMemoryDb.collection('spectators').remove({}, function(err, res){
		console.log('DATABASE: In memory (cache) spectators record have been removed successfully!');
	});

	// Reset player join record as false to previous records
	mongodb.inMemoryDb.collection('tableJoinRecord').remove({}, function(err, res){
		console.log('DATABASE: Player records of table joined have been removed successfully!');
	});

	// Delete inmemory tournament rank collection
	mongodb.inMemoryDb.collection('ranks').remove({}, function(err, res){
		console.log('DATABASE: Dynamic rank for tournament records have been removed successfully!');
	});

	// reset online players count
	mongodb.inMemoryDb.collection('onlinePlayers').update({},{"onlinePlayers" : 0}, {upsert: true}, function(err, result) {
	...
	mongodb.inMemoryDb.collection('userActivity').remove({}, function(err, res){
		console.log('DATABASE: User activity of joined table been removed successfully!');
	});
	...
	mongodb.db.collection('tournamentroom').update({}, {$set: {state: "REGISTER"}}, {multi: true}, function(err, res){
		console.log('DATABASE: Tables for tournament have been removed successfully!');
	});


	// Remove existing tournament users
	mongodb.db.collection('antibanking').remove({}, function(err, res){
		console.log('DATABASE: Anti bankign details for users have been removed successfully!');
	});
	...
	mongodb.db.collection('rebuy').remove({}, function(err, res){
		console.log('DATABASE: rebuy for tournament have been removed successfully!');
	});

	// Remove existing tournament ranks
	mongodb.db.collection('tournamentRanks').remove({}, function(err, res){
		console.log('DATABASE: Users rank records for tournament  been removed successfully!');
	});
	...
	mongodb.db.collection('antibanking').createIndex( { "createdAt": 1 }, { expireAfterSeconds: configConstants.expireAntiBankingSeconds});

	mongodb.db.collection('userSession').remove({});

	mongodb.db.collection('scheduleTasks').updateMany({type: 'serverDown', status: 'PENDING'}, {$set: {status: 'CANCELLED'}, $push: {logs: ("schedule cancelled, server started, "+ new Date())}}, function (err, result) {

	});
	mongodb.db.collection('scheduleTasks').updateMany({type: 'serverUp', status: 'PENDING'}, {$set: {status: 'CANCELLED'}, $push: {logs: ("schedule cancelled, server started, "+ new Date())}}, function (err, result) {
 ...
 //  });
	...
	mongodb.logDB.collection('videos').createIndex({"roundId" : 1, "channelId" : 1},{background: true}, function(err, result){
		console.log(JSON.stringify(err)+" Create Index on videos " + JSON.stringify(result));
	});
};
