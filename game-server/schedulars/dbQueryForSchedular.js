...
var MongoClient 		= require('mongodb').MongoClient,
		...
		schedulerConfig = require("./schedulerConfig"),
...
exports.findAllTournamentRoom = function(callback) {
	mongodb.db.collection('tournamentroom').find({$or:[{tournamentType: schedulerConfig.tournamentType.normal},{tournamentType: schedulerConfig.tournamentType.satelite}]}).toArray(function (err, result) {
		callback(err, result);
	});
};

exports.updateTournament = function(tournamentIds,callback) {
	mongodb.db.collection('tournamentroom').update({_id: {$in: tournamentIds}},{$set: {state: schedulerConfig.tournamentState.register,tournamentStartTime: 0}, $inc:{gameVersionCount: 1}},{multi : true}, function(err, result) {
		callback(err, result);
	});
};

exports.updateTournamentRoom = function(query, updatedKeys, callback) {
  mongodb.db.collection('tournamentroom').findAndModify(query,[],{$set: updatedKeys}, {new: true}, function (err, result) {
    callback(err, result);
  });
};

exports.updateSingleTournament = function(tournamentId,callback) {
	mongodb.db.collection('tournamentroom').update({_id: ObjectID(tournamentId)},{$set: {state: schedulerConfig.tournamentState.register}, $inc:{gameVersionCount: 1}}, function(err, result) {
		callback(err, result);
	});
};

exports.updateTournamentStateToRunning= function(tournamentId,callback) {
	mongodb.db.collection('tournamentroom').update({_id: ObjectID(tournamentId)},{$set: {state: schedulerConfig.tournamentState.running}}, function(err, result) {
		callback(err, result);
	});
};
...
exports.saveJob = function(job,callback) {
	mongodb.db.collection('jobs').insert({tournamentId:1,job:job}, function(err, result) {
		console.log(result);
		callback(err, result);
	});
};
...
exports.removeTables = function(tournamentId, cb){
	mongodb.db.collection('tables').remove({"tournament.tournamentId": tournamentId.toString()},function(err, result) {
		cb(err, result);
	});
};

exports.deletePrize = function(tournamentId, cb){
	mongodb.db.collection('prizerules').remove({tournamentId : tournamentId.toString(),type: "server"},function(err, result) {
		cb(err, result);
	});
};
