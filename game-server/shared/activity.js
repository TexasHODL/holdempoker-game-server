...
userActivity.logUserActivity = function (params,category,subCategory,status) {
	var activityObject = {};
...
userActivity.getLobbyTables = function(rawInput,category,subCategory,rawResponse,logType){
...
userActivity.getTable = function(rawInput,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
	if(!!rawInput){
		activityObject.channelType = !!rawInput.channelType ? rawInput.channelType : "NORMAL";
		activityObject.rawInput = rawInput;
...
userActivity.updateProfile = function(rawInput,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
...
userActivity.lobbyRegisterTournament = function(rawInput,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
	activityObject.rawInput = rawInput;
	activityObject.rawResponse = rawResponse;
	activityObject.playerId = rawInput.playerId;
...
userActivity.lobbyDeRegisterTournament = function(rawInput,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
	activityObject.rawInput = rawInput;
	activityObject.rawResponse = rawResponse;
	activityObject.playerId = rawInput.playerId;
	activityObject.channelId 	= (rawInput.channelId || "");
	if(rawResponse.success){
		activityObject.comment = "Player de-registered successfully for tournament.";//+ rawInput.tournamentId;
	} else{
...
userActivity.playerSit = function(rawResponse,category,subCategory,logType){
	var activityObject = init(category,subCategory,logType);
	if(!!rawResponse){
		...
		if(!!rawResponse.player){
			...
			if(rawResponse.response.success){
				activityObject.comment = rawResponse.player.playerName + " sat on seat " + rawResponse.player.seatIndex + " with " + rawResponse.player.chips + " chips in "+ rawResponse.player.state + " state ";
...
userActivity.makeMove = function(params,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
	activityObject.channelId 	= (params.channelId || "");
	if(!!rawResponse){
		// activityObject.rawInput 		= rawInput;
		...
		if(!!params.data && rawResponse.success){
			...
			activityObject.comment+= " Pre Chips = " + preChips + ". Post Chips = " + params.data.chips;
		...
		if(!!params.table.players){
			async.each(params.table.players,function(player,ecb){
				...
				if(logType === stateOfX.logType.success){
					...
					activityObject.comment = "Could not fetch player information-"+rawResponse.info;
				}
				if(category === stateOfX.profile.category.gamePlay){
...
userActivity.winner = function(params,category,subCategory,rawResponse,logType){
	if(!!params){
		async.each(params.table.players,function(player,ecb){
			var activityObject = init(category,subCategory,logType);
			activityObject.rawResponse = rawResponse;
			...
			if(rawResponse.success){
				...
				var winner = _.findWhere(params.data.winners,{playerId:player.playerId});
				if(!!winner){
					// activityObject.comment			= player.playerName + " won " + winner.amount + " chips by "+winner.type + " ("+winner.typeName + ")."
					activityObject.comment			= player.playerName + " won " + winner.amount + " chips by "+winner.type + ".";
				} else{
...
userActivity.startGame = function(params,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
...
userActivity.gameOver = function(params,category,subCategory,rawResponse,logType){
	var activityObject = init(category,subCategory,logType);
	activityObject.rawResponse = rawResponse;
	activityObject.channelId 	= (params.channelId || "");
	...
	if(logType === stateOfX.logType.success){
		...
		if(!!rawResponse.params.table.channelType && !!rawResponse.params.table.channelName && !!rawResponse.params.table.channelVariation){
		...
		_.each(params.data.contributors,function(contributor){
			var player = _.findWhere(params.table.players,{"playerId":contributor.playerId});
			if(!!player){
		...
		async.each(params.table.pot,function(pot,ecb){
			...
			_.each(params.table.pot[pot.potIndex].contributors,function(contributor){
				var player = _.findWhere(params.table.players,{"playerId":contributor});
				if(!!player){
...
userActivity.rakeDeducted = function(params,category,subCategory,logType){
	...
	if(logType === stateOfX.logType.success){
		...
		var player = _.where(params.table.players, {playerId: Object.keys(params.rakeDetails.playerWins)[0]});
		var contribute = _.where(params.table.contributors, {playerId: player[0].playerId});
...
userActivity.sitoutNextHand = function(params,category,subCategory,logType){
	var activityObject = init(category,subCategory,logType);
	var player = _.where(params.table.players, {playerId: params.data.playerId});
	if(!!params){
		activityObject.playerId = (params.data.playerId || "") ;
...
userActivity.sitoutNextBigBlind = function(params,category,subCategory,logType){
	var activityObject = init(category,subCategory,logType);
	var player = _.where(params.table.players, {playerId: params.data.playerId});
	if(!!params){
		activityObject.playerId = (params.data.playerId || "") ;
...
userActivity.addChipsOnTable = function(params,category,subCategory,logType,playerChipDetails){
	var activityObject = init(category,subCategory,logType);
	var player = _.where(params.table.players, {playerId: params.data.playerId});
	if(!!params && !!playerChipDetails){
		...
		if(!playerChipDetails.isChipsToUpdate){
			activityObject.comment = player[0].playerName + ": addded chip(s) " + playerChipDetails.newChips+ " to " + playerChipDetails.chipsInHand + " in " + params.table.channelName + " has " + player[0].chips + " chips.";
		}else{
			activityObject.comment = player[0].playerName + ": added chip(s) " + playerChipDetails.newChipsToAdd+ " for next game in " + params.table.channelName + " will have " + player[0].chipsToBeAdded + " more chips. and currently has "+ playerChipDetails.chipsInHand + "chips in hand.";
		}
	}
	// insertInDb(activityObject,function(){});
	insertInDbGame(activityObject,function(){});
...
userActivity.updateTableSettings = function(params,category,subCategory,logType,playerName){
	var activityObject = init(category,subCategory,logType);
...
userActivity.logWinnings = function (channelType, channelVariation, channelId, timestamp, winners, contributors) {
	for (var i = 0; i < winners.length; i++) {
		var winner = winners[i];
		var t = _.findWhere(contributors, {playerId: winner.playerId}) || {amount: 0};
		var amount = winner.amount - t.amount;
		var logObject = {channelType: channelType, channelVariation: channelVariation, channelId: channelId, timestamp: timestamp, playerId: winner.playerId, amount: amount};
