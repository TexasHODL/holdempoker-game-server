...
var tableKeys = function(params) {
  ...
  var tempObj ={
    ...
    'numberOfRebuyAllowed'      : !!params.room.numberOfRebuyAllowed?params.room.numberOfRebuyAllowed:null,
    'hourLimitForRebuy'         : !!params.room.hourLimitForRebuy?params.room.hourLimitForRebuy:null,
    'rebuyHourFactor'           : !!params.room.rebuyHourFactor?params.room.rebuyHourFactor:null,
...
var deleteExistingTables = function(params, cb) {
  console.log("room is in deleteExistingTables - ",JSON.stringify(params.room));
...
var getBreakRule = function(params,cb){
  db.findBreakRule(params.room.breakRuleId,function(err,result){  //getBreakRule according to the breakRuleId
      if(!err && result){
        serverLog(stateOfX.serverLogType.info,"The result in getBreakRule is  - " + JSON.stringify(result));
        params.breakRuleData = result;     //send the breakRule Data to the function isEligible for break
        cb(null,params);
      }else{
        serverLog(stateOfX.serverLogType.info,"Some error occured or break Rule not found" );
        cb({ success: false});
      }
    });
...
var getBlindRule = function(params,cb){
  db.findBlindRule(params.room.blindRule,function(err,result){  //getBlindRule according to the BlindRuleId
      if(!err && result){
        serverLog(stateOfX.serverLogType.info,"The result in getBlindRule is  - " + JSON.stringify(result));
...
var getTimeBankRule = function(params,cb){
  db.findTimeBankRule(params.room.timeBankRule,function(err,result){  //getTimeBank according to the TimeBankId
      if(!err && result){
        serverLog(stateOfX.serverLogType.info,"The result in getTimeBankRule is  - " + JSON.stringify(result));
...
table.create = function(room,cb) {
  ...
  async.waterfall([async.apply(initializeParams, room),
    enrolledPlayers,
...
var getTournamentRoom = function(params, cb)  {
  ...
  db.getTournamentRoom(params.tournamentId, function(err, result) {
      if(err) {
        ...
        console.log("room is in getTournamentRoom is - " + JSON.stringify(result));
        params.room = result;
        cb(null, params);
      }
  });
...
table.createTableByTournamentId = function(tournamentId, cb) {
  ...
  async.waterfall([async.apply(getTournamentRoom, params),
    enrolledPlayers,
    getBlindRule,
    getBreakRule,
    getTimeBankRule,
