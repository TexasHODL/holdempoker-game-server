...
var channelStore = require("./app/util/channelStoreDB")(
  "channelStoreColl",
  "db"
);
...
db.init(app.serverType, function (dbError) {
  ...
  redisManager.dbConnect({}, (connectFailed, client) => {
    ...
    console.log(
      "Host: " +
        app.get("curServer").host +
        " & Port: " +
        (app.get("curServer").port || app.get("curServer").clientPort)
    );
    ...
    app.configure("production|development", function () {
      ...
      app.use(require("./plugins/mailer"), {
        devMailer: {
          key: stateOfX.SendGridApiKey,
          prefix: "Dev",
          fromName: configConstants.mailFromAppName,
        },
      });
    });
    ...
    app.start(function () {
      app.event.on("TestServerException", function (datas) {
        ...
        app.rpc.connector.entryRemote.sendMailToDevelopers(
          "SESSION",
          datas,
          function (response) {
