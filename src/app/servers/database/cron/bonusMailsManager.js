...
const prepareMailDataOfDetails = (player, cb) => {
  ...
  var dailyStatusTableForEmail =
    '<html><body><table style="font-size: 11px; margin: 15px 0; border-collapse: collapse;">  <tr>    <th width="25%" align="center" style="border: 1px solid;">Locked Bonus </th>    <th width="25%" align="center" style="border: 1px solid;">Expiry Date </th>    <th width="25%" align="center" style="border: 1px solid;">Points Remaining </th> </tr> ';
  console.log("inside prepare mail data for player", player);
  for (let i = 0; i < player.bonusData.length; i++) {
    let pointsRemaining = 0;
    pointsRemaining =
      player.bonusData[i].lockedBonusAmount -
      player.playerInfo.statistics.countPointsForBonus;
    if (pointsRemaining > 0) {
      ...
      dailyStatusTableForEmail =
        ...
        new Date(
          player.bonusData[i].expireAt + 330 * 60 * 1000
        ).toLocaleString() +
        ' (IST)</td> <td width="25%" align="center" style="line-height: 20px; border: 1px solid;">' +
        pointsRemaining.toFixed(4) +
        "</td> </tr>";
    }
  }
  dailyStatusTableForEmail =
    dailyStatusTableForEmail + "</table></body></html><br><br>";
  if (isSendDetails) {
    const content = {};
    content.name = player.playerInfo.userName;
    content.dailyBonusDetails = dailyStatusTableForEmail;
    const receiverSubject =
      "Hurry Up!! Pokermoogley Locked Bonus will expire soon.";
    const mailData = createMailData({
      content: content,
      toEmail: player.playerInfo.emailId,
      from_email: configConstants.from_email,
      subject: receiverSubject,
      template: "dailyUpdateLockedBonus",
    });
...
const prepareMailDataOfClaim = (player, cb) => {
  ...
  var claimBonusTable =
    '<html><body><table style="font-size: 11px; margin: 15px 0; border-collapse: collapse;">  <tr>    <th width="25%" align="center" style="border: 1px solid;">Locked Bonus </th>    <th width="25%" align="center" style="border: 1px solid;">Expiry Date </th> </tr> ';
  for (let i = 0; i < player.bonusData.length; i++) {
    let pointsRemaining = 0;
    pointsRemaining =
      player.bonusData[i].lockedBonusAmount -
  ...
  if (isClaimToSend) {
    ...
    const mailData = createMailData({
      content: content,
      to_email: player.playerInfo.emailId,
      from_email: configConstants.from_email,
      subject: subject,
      template: "claimBonusMails",
    });
    sendMailToPlayers(mailData, function (mailResponse) {
      console.log("Mail sent successfully");
      cb(null, player);
...
const forEveryPlayerDetailsMail = (params, cb) => {
  async.each(
    params.playerBonusData,
    function (player, ecb) {
      async.waterfall(
        [async.apply(getPlayerInfo, player), prepareMailDataOfDetails],
...
BonusMailsManager.prototype.bonusDailyUpdateMails = () => {
  ...
  async.waterfall(
    [async.apply(getPlayerActiveBonus, {}), forEveryPlayerDetailsMail],
    function (err, result) {
...
const forEveryClaimMails = (params, cb) => {
  async.each(
    ...
    function (player, ecb) {
      async.waterfall(
        [async.apply(getPlayerInfo, player), prepareMailDataOfClaim],
        function (err, eachResult) {
          ecb(null);
        }
      );
...
BonusMailsManager.prototype.lockedBonusClaimMails = () => {
  ...
  async.waterfall(
    [async.apply(getPlayerActiveBonus, {}), forEveryClaimMails],
    function (err, result) {
