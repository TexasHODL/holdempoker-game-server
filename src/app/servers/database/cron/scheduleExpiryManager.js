  ...
  db.findUsersOpts(
    { isBot: false },
    {
      fields: {
        ...
        userName: 1,
        realChips: 1,
        "statistics.megaPoints": 1,
        "statistics.megaPointLevel": 1,
        "statistics.handsWonRM": 1,
        "statistics.handsPlayedRM": 1,
      },
    },
    function (err, result) {
      if (err) {
        ...
        [
          {
            ...
            megaPoints: user.statistics.megaPoints,
            megaPointLevel: user.statistics.megaPointLevel,
            handsWon: user.statistics.handsWonRM,
            handsPlayed: user.statistics.handsPlayedRM,
          },
        ],
        function (err, res) {
...
var savePlayerKeys = function () {
  ...
  async.waterfall(
    [async.apply(getUsers, { timestamp: timestamp }), forEveryUser],
    function (err, params) {
...
const getExpiryCrossedSlots = (params, cb) => {
  ...
  db.findExpirySlots(
    { expireStatus: 0, expireAt: { $lt: currentTime } },
    function (err, res) {
      console.log(err);
      console.log(res);
      if (err || !res) {
        ...
        if (res instanceof Array && res.length > 0) {
          ...
          cb(null, {
            success: true,
            info: "No one is going to loose Locked bonus tonight, so yes WORK DONE!",
          });
          return;
        }
      }
 ...
 */
function checkThresholdCrossed(levels, levelId, oldPoints, subPoints) {
  // Boolean
...
const checkIfAvailableToClaim = (slot, params, cb) => {
  ...
  if (
    slot.playerInfo &&
    slot.playerInfo.statistics.countPointsForBonus >= slot.lockedBonusAmount
  ) {
    slot.isClaim = true; // Release
    slot.levelChange = checkThresholdCrossed(
      params.allLevels,
      slot.playerInfo.statistics.megaPointLevel,
      slot.playerInfo.statistics.megaPoints,
      slot.lockedBonusAmount
    ); // Boolean
    cb(null, slot, params);
  } else {
...
const addChipsPlayerRelease = (slot, params, cb) => {
  ...
  if (slot.isClaim) {
    ...
    db.updateUserBalance(
      chipsData,
      slot.playerId,
      function (err, playerUpdateResult) {
        if (err) {
...
const updatePlayerBonusData = (slot, params, cb) => {
  ...
  updateData["bonus.$.expiredAmt"] = slot.isClaim
    ? 0
    : slot.bonusData.unClaimedBonus;
  updateData["bonus.$.claimedBonus"] = slot.isClaim
    ? slot.bonusData.unClaimedBonus
    : 0;
  db.updateBounsDataSetKeys(query, updateData, function (err, result) {
    if (err) {
      ...
      db.findUserBonusDetails(
        { playerId: slot.playerInfo.playerId },
        function (err, bonusResult) {
...
const updatePlayerStats = (slot, params, cb) => {
  if (slot.isClaim) {
    ...
    updateData["statistics.megaPoints"] =
      slot.playerInfo.statistics.megaPoints - slot.bonusData.unClaimedBonus;
    updateData["statistics.countPointsForBonus"] =
      slot.playerInfo.statistics.countPointsForBonus -
      slot.bonusData.unClaimedBonus;
    if (slot.levelChange) {
...
const updateSlotStatusExpired = (slot, params, cb) => {
  ...
  if (slot.isClaim) {
    ...
    dataToUpdate.newVipPoints =
      slot.playerInfo.statistics.megaPoints - slot.bonusData.unClaimedBonus;
    dataToUpdate.prevVipLevel = getLevelName(
      slot.playerInfo.statistics.megaPointLevel,
      params.allLevels
    );
    ...
    if (slot.levelChange) {
      dataToUpdate.newVipLevel = getLevelName(
        slot.levelChange.value,
        params.allLevels
      );
    } else {
      dataToUpdate.newVipLevel = getLevelName(
...
const broadcastPlayerData = (slot, params, cb) => {
  ...
  if (slot.isClaim) {
    broadcastData.realChips =
      slot.playerInfo.realChips +
      slot.playerInfo.instantBonusAmount +
      slot.bonusData.unClaimedBonus;
    broadcastData.unclamedBonus = slot.prevUnclaimed;
    broadcastData.megaPoints =
      slot.playerInfo.statistics.megaPoints - slot.bonusData.unClaimedBonus;
    if (slot.levelChange) {
      broadcastData.megaPointLevel = getLevelName(
        slot.levelChange.value,
        params.allLevels
      );
    }
    broadcastData.megaPointsPercent = getLevelPercent(
      slot.playerInfo.statistics.megaPoints - slot.bonusData.unClaimedBonus,
      params.allLevels
    );
  } else {
  ...
  pomelo.app.rpc.connector.sessionRemote.broadcastPlayer(
    "",
    {
      data: playerData,
      route: "updateProfile",
...
const sendMailMessagesToPlayer = (slot, params, cb) => {
  console.log(
    "inside sendMailMessagesToPlayer slot data " + JSON.stringify(slot)
  );
  ...
  if (slot.isClaim) {
    ...
    mailData.previousChips = fixedDecimal(
      slot.playerInfo.realChips + slot.playerInfo.instantBonusAmount,
      2
    );
    mailData.updatedChips = fixedDecimal(
      slot.playerInfo.realChips +
        slot.playerInfo.instantBonusAmount +
        slot.bonusData.unClaimedBonus,
      2
    );
    mailData.playerPrevVipPoints = fixedDecimal(
      slot.playerInfo.statistics.megaPoints,
      4
    );
    mailData.playerNewVipPoints = fixedDecimal(
      slot.playerInfo.statistics.megaPoints - slot.bonusData.unClaimedBonus,
      4
    );
    ...
    mailData.playerPrevVipLevel = getLevelName(
      slot.playerInfo.statistics.megaPointLevel,
      params.allLevels
    );
    if (slot.levelChange) {
      mailData.playerNewVipLevel = getLevelName(
        slot.levelChange.value,
        params.allLevels
      );
    } else {
    ...
    msg =
      ...
      slot.playerInfo.userName +
      ",\n" +
      "Your locked bonus of " +
      slot.bonusData.unClaimedBonus +
      " credited on " +
      new Date(slot.createdAt + 330 * 60 * 1000).toLocaleString() +
      " has expired due to insufficient VIP points, the last date to accumulate the required VIP points was " +
      new Date(slot.expireAt + 330 * 60 * 1000).toLocaleString();
    messageData = {
      mobileNumber: "91" + slot.playerInfo.mobileNumber,
      msg: msg,
    };
...
const sendBonusExpiredOrReleaseMail = function (params) {
  pomelo.app.rpc.connector.adminManagerRemote.sendMailToPlayers(
    "",
    params,
    function (bonusMailResponse) {
...
const sendBonusExpiredOrReleaseMessage = function (params) {
  pomelo.app.rpc.connector.adminManagerRemote.sendMsgToPlayers(
    "",
    params,
    function (bonusMessageResponse) {
