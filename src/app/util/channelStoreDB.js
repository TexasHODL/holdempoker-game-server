...
channelStoreDB.prototype.remove = function (key, value, cb) {
  ...
  this.db[this.dbName]
    .collection(this.col)
    .update(query, { $pull: update }, function (err, res) {
      // console.log('channelStoreDB.prototype.add - response', err, res)
      cb(err);
    });
};
...
channelStoreDB.prototype.updateKeys = function (app, channel) {
  ...
  var setChannelKeys = function (channel, table) {
    ...
    if (channel.isTable) {
      ...
      channel.waitingPlayers =
        table.queueList instanceof Array ? table.queueList.length : 0;
      ...
      if (table.state == "RUNNING") {
        ...
        channelTimerHandler.startTurnTimeOut({
          event: "afterCrash",
          self: selfa,
      ...
      for (var i = 0; i < table.players.length; i++) {
        ...
        if (table.players[i].state == "RESERVED") {
          channelTimerHandler.vacantReserveSeat({
            event: "afterCrash",
            self: selfa,
            channel: channel,
            channelId: channel.name,
            playerId: table.players[i].playerId,
            session: "session",
          });
        }
      }
      for (var i = 0, list = channel.getMembers(); i < list.length; i++) {
        ...
        if (flag) {
          ...
          channelTimerHandler.kickPlayerToLobby({
            event: "afterCrash",
            self: selfa,
            channel: channel,
            channelId: channel.name,
  ...
  var getTable = function (app, channel, table) {
    app.rpc.database.tableRemote.getTable(
      "session",
      { channelId: channel.name },
