...
module.exports.redirConn = function (session, msg, app, cb) {
  ...
  if (msg.namespace == "user") {
    if (
      ["entryRemote", "sessionRemote", "adminManagerRemote"].indexOf(
        msg.service
      ) >= 0
    ) {
...
module.exports.redirect = function (session, msg, app, cb) {
  ...
  if (msg.namespace == "sys" && msg.method == "forwardMessage") {
    access_token = msg.args[0].body.access_token
      ? msg.args[0].body.access_token
      : " ";
      ...
      if (isFirstRoute(msg)) {
        getRedirectFromDB(channelId, function (sserverId) {
          if (sserverId) {
            ...
            cb(null, server.id);
            return; // its random it needs to be changed
          }
        });
      } else {
...
function dispatcherNeeded(msg) {
  ...
  if (msg.namespace == "sys" && msg.method == "forwardMessage") {
    ...
    return (
      ["broadcastRemote", "logoutRemote", "roomRemote"].indexOf(msg.service) >=
      0
    );
  }
}
...
function isFirstRoute(msg) {
  ...
  if (msg.namespace == "sys" && msg.method == "forwardMessage") {
    var actualMsg = msg.args[0];
    return (
      "room.channelHandler.joinChannel" === actualMsg.route ||
      "room.channelHandler.autoSit" === actualMsg.route
    );
  } else if (msg.namespace == "user") {
    return (
      ["broadcastRemote", "logoutRemote", "roomRemote"].indexOf(msg.service) >=
        0 && ["joinChannel", "autoSit"].indexOf(msg.method) >= 0
    );
  }
}
...
function isLastRoute(msg) {
  ...
  if (msg.namespace == "sys" && msg.method == "forwardMessage") {
    ...
    return (
      ["broadcastRemote", "roomRemote"].indexOf(msg.service) >= 0 &&
      ["leaveRoom"].indexOf(msg.method) >= 0
    );
  }
}
...
function removeRedirectFromSession(session, channelId) {
  ...
  console.log(
    "getRedirectFromSession-1",
    channelId,
    actualConnSession.settings
  );
  if (!channelsMap) {
    return false;
  }
