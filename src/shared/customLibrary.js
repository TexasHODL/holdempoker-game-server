...
custumLibrary.pluckKeys = function (collectionArray, keysArray) {
  ...
  if (
    ...
    for (
      ...
      collectionIterator < collectionArray.length;
      collectionIterator++
    ) {
      var tempObject = {};
      for (
        var keysIterartor = 0;
        keysIterartor < keysArray.length;
        keysIterartor++
      ) {
        tempObject[keysArray[keysIterartor]] =
          collectionArray[collectionIterator][keysArray[keysIterartor]];
      }
    ...
    for (
      ...
      for (
        ...
        if (!!collectionArray[collectionIterator][keysArray[keysIterartor]]) {
          sum =
            sum + collectionArray[collectionIterator][keysArray[keysIterartor]];
        }
      }
    ...
    if (!!collectionArray && collectionArray.length > 1 && !!object) {
      for (
        ...
        for (
          ...
          if (!!collectionArray[collectionIterator][keysArray[keysIterartor]]) {
            collectionArray[collectionIterator][keysArray[keysIterartor]] =
              object[keysArray[keysIterartor]];
          }
        }
      }
    }
...
custumLibrary.convertToJson = function (json) {
  for (var obj in json) {
    ...
    for (var keys in json[obj]) {
      // first level keys
      for (var key in json[obj][keys]) {
        // second level keys
        for (var k in json[obj][keys][key]) {
          // third level keys
...
var convert = function (k, json) {
  if (k.indexOf(".") != -1) {
    // if key contains '.' i.e. to be splitted
    var a = k.split(".");
    var j = {};
